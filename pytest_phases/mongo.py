...
class MongoConnector(object):
    ...
    def __init__(self, enable, hosts, db_name, replica_set):
        if enable:
            if replica_set:
                client = MongoClient(host=hosts, replicaset=replica_set)
            else:
                client = MongoClient(host=hosts)
    ...
    @staticmethod
    def split_to_chunks_at_write_limit(msg_data):
        # Split a list (data) into smaller lists of length equal to the
        # MongoDB write limit.
        mongo_write_limit = 100000
        chunks = []
        counter = 0
        while counter < len(msg_data):
            end_of_chunk = counter + mongo_write_limit
            chunks.append(msg_data[counter:end_of_chunk])
            counter += mongo_write_limit
        return chunks
    ...
    def bulk_insert_log_messages(self, msgs_log_params):
        ...
        log_chunks = self.split_to_chunks_at_write_limit(msgs_log_params)
